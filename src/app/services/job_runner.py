...
from ..models import JobApplication, Resume, JobStatus
...
async def _run_job_with_session(db: AsyncSession, job_id: str) -> None:
    ...
    def finalize(state: AgentState) -> AgentState:
        ...
        try:
            ...
            job.result_summary = result.get("summary") or result.get("result", {}).get("summary")
            job.live_view_url = result.get("liveViewUrl") or result.get("result", {}).get("liveViewUrl")
            job.error = None
    ...
    try:
        await graph.ainvoke(AgentState({"url": job.target_url}))
        await db.commit()
    except Exception as e:  # noqa: BLE001
        job.status = JobStatus.failed
        job.error = str(e)
